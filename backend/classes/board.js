class Board {
  ...
  initializeFromPawns(pawns, currentPlayerIdToColor) {
    ...
    pawns.forEach((pawn) => {
      const color = currentPlayerIdToColor.get(pawn.player_ID);
      ...
      if (color !== pawnColor) {
        ...
        if (position === this.slidePositions[color].small.start) {
          return {
            ...
            slideType: "small",
            slideLength: 4,
            start: this.slidePositions[color].small.start,
            end: this.slidePositions[color].small.end,
          };
        }
        ...
        if (position === this.slidePositions[color].large.start) {
          return {
            ...
            slideType: "large",
            slideLength: 5,
            start: this.slidePositions[color].large.start,
            end: this.slidePositions[color].large.end,
          };
        }
      }
    }
  ...
  swapPawnPositions(
    ...
    let pawn1Color = this.getPawnAtPosition(
      pawn1Position,
      pawn1Zone,
      pawn1ZoneColor
    );
    let pawn2Color = this.getPawnAtPosition(
      pawn2Position,
      pawn2Zone,
      pawn2ZoneColor
    );
